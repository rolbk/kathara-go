...
Pins the observable behaviour of
  ...
  * ``Kathara.utils.convert_win_2_linux(filename, write=False|True)``
  * ``binaryornot.check.is_binary(filename)``  (the exact version resolved in
    the oracle venv -- see the ``binaryornot`` block of ``expected.json``)

over a fixed fixture corpus, and dumps the result to
``internal/util/testdata/encoding/expected.json`` for the Go table test in
``internal/util/encoding_test.go``.

The Python behaviour is the truth. When Go disagrees with ``expected.json``,
Go is wrong -- unless the disagreement is one of the impossible-parity items
recorded in ``docs/port/SPIKES/encoding.md``, in which case the finding gets
promoted to DIVERGENCES.md.
...
Usage:
    ...
    /root/kathara/pyvenv/bin/python tools/vectorcheck/encoding_probe.py --legacy DIR

Options:
    --regen-corpus   rewrite the fixture files from the CORPUS table below,
                     then probe. The fixtures are committed, so a plain run
                     probes whatever is on disk; use this only when adding a
                     fixture.
    --out PATH       expected.json destination (default: the testdata dir)
    --legacy DIR     directory containing an unpacked ``binaryornot`` 0.4.4
                     (``pip download binaryornot==0.4.4 --no-deps -d X`` then
                     unzip the wheel). When given, each fixture also records
                     ``is_binary_v044`` so the version divergence documented in
                     SPIKES/encoding.md can be re-measured.
    --verbose        print one line per fixture.

Exit status is 0 when the corpus on disk matches the CORPUS table byte for
byte and every probe completed.
"""
...
def probe_one(name: str, tmpdir: str, legacy) -> dict:
    ...
    # so the Go port can be diffed feature-by-feature when a verdict disagrees,
...
def main() -> int:
    ...
    doc = {
        "_comment": (
            ...
            "Do not hand-edit. See docs/port/SPIKES/encoding.md."
        ),
